...
function cargar(){
    ...
    if (file) {
        ...
        reader.onload = function (evt) {
            ...
            fetch('http://35.192.14.170:5000/carga', {
            method: 'POST',
            headers,
...
modificarlibro(){
    ...
    fetch('http://35.192.14.170:5000/libros/'+titulo_o.value, {
      method: 'PUT',
      headers,
      body: reque,
    })
...
function eliminar(libro,autor){
    ...
    fetch('http://35.192.14.170:5000/libros/'+libro+'/'+autor,{
        method:'DELETE'
    })
    .then(res => res.text())
    .then(res=> {
...
function agregarlibro(){
  ...
  fetch('http://35.192.14.170:5000/libros', {
    method: 'POST',
    headers,
    body: `{
        "titulo":"${titulo.value}",
...
let text="";
fetch('http://35.192.14.170:5000/obtenerlibros')
.then(response => response.json())
.then(data =>{
    var i;
...
fetch('http://35.192.14.170:5000/obtenerusuarios')
.then(response => response.json())
.then(data =>{
  var i;
...
//aqui agrego las cartas de los pacientes
    document.getElementById("cardsc").innerHTML = '';
    let text="";
    fetch('http://35.192.14.170:5000/obtenerPacientes')
    .then(response => response.json())
    .then(data =>{
        var i;
        for(i=0;i<data.length;i++){
